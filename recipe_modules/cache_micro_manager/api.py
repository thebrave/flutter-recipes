...
PACKAGE_REMOVAL_INTERVAL = 30 #days
...
class CacheEntry:
  ...
  def _calc_removal_date(self, updated_date: datetime):
    ...
    time_delta = timedelta(days=PACKAGE_REMOVAL_INTERVAL)
...
class CacheMicroManagerApi(recipe_api.RecipeApi):
      ...
      if not self.m.path.exists(self.cache_metadata_file_path):
        ...
        for dep in deps_list:
          if not self.is_file_name_in_cache_entry_list(
            ...
            # we need to add it.
            current_metadata_entries.append(CacheEntry(file_name=dep, updated_date=self.today()))
          else:
            ...
            new_entry = CacheEntry(file_name=existing_entry.name, updated_date=self.today())
        ...
        for current_meta_entry in current_metadata_entries:
          if current_meta_entry.name not in [
        ...
        for file_name_str in current_directory_entries:
          ...
          if cacheEntry is None:
            current_metadata_entries.append(
                CacheEntry(file_name=self.m.path.abspath(file_name_str), updated_date=self.today())
            )
  ...
  def convert_file_list_to_cache_entry_list(
    ...
    for fd in file_names_list:
      cache_entry = CacheEntry(file_name=self.m.path.abspath(fd), updated_date=self.today())
    ...
    with self.m.step.nest('Reading metadata file {}'.format(
      ...
      for item in json_items:
        ...
        new_entry = CacheEntry(
            file_name=item[JSON_ENTRY_NAME], updated_date=updated_date_found, removal_date=removal_date_found
        )
